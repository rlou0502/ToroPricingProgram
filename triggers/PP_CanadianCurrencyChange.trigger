trigger PP_CanadianCurrencyChange on REVVY__MnQuote__c (after update) {
    ...
    system.debug(logginglevel.info, 'PP_CanadianCurrencyChange -- quoteIds =' + quoteIds);
    system.debug(logginglevel.info, 'PP_CanadianCurrencyChange -- canadianExchangeRatios =' + canadianExchangeRatios);
    if(quoteIds.size() > 0) {
	    ...
	    system.debug(logginglevel.info, 'PP_CanadianCurrencyChange -- quoteItems =' + quoteItems);
